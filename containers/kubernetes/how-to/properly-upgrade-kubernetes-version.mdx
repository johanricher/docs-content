---
meta:
title: How to properly upgrade your Kubernetes Kapsule cluster
description: This page explains how to properly upgrade your Kubernetes Kapsule cluster
content:
h1: How to properly upgrade your Kubernetes Kapsule cluster
...
<Message type="requirement">
    ...
    - You have a Kubernetes Kapsule cluster which is not running the latest k8s version available on Kapsule API and
    you're willing to upgrade it.
</Message>

First, we'll make sure your workload can be properly handled by the newer Kapsule version. We have a compatibility matrix
for different components and your current cluster may use components that are deprecated or not available on the new version.
Please see our [version policy](https://www.scaleway.com/en/docs/containers/kubernetes/reference-content/version-support-policy/).

For each version upgrade, the client is encouraged to read kubernetes [changelog](https://kubernetes.io/releases/).

## Checking which components must be changed

An easy way to visualize this is with the command `scw k8s version list`.
e.g.
```
...
Managed Ingresses are deprecated since the minor release 1.24, we provide the feature EasyDeploy instead to set up an ingress controller.
...
3. In parallel, you can start cordoning nodes using the old runtime. this way, workload will get rescheduled directly on the containerd nodes.
...
4. Once the new nodes are ready, you can start draining the old nodes, you may need to add the option `--delete-emptydir-data` if you used local disk as a scratchpad.
  `kubectl drain --ignore-daemonsets $NODE_TO_DRAIN`
    <Message type="important">
        DO NOT drain all the nodes simultaneously, prefer doing it sequentially, making sure your workload is behaving as expected
    </Message>
...
If your cluster is using a deprecated CNI, and you're willing to upgrade to a newer k8s version, you will most likely have to spin up a new Kapsule cluster.
We do not provide (as of now) an easy way to change CNI. This component is integrated in each cluster node which results in a tricky transition.
Do not hesitate to contact us on our Slack community channel #k8s or open a support ticket if you're in this situation and need help.

## Effective Upgrade

So, we have two possible paths from here, you're either upgrading one minor version or multiple ones.
...
<Message type="important">
    Your pools and your control plane SHOULD NEVER have more than a one-minor-version delta or you can expect things to break
</Message>
